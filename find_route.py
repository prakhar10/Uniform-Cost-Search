...
def expand_nodes(data_dictionary,fringe,visited_nodes,destination):
    ...
    if len(fringe) is not 0:
        node = fringe[0]
        #print("current node:"+node.get_state())
        fringe.remove(node)
        ...
        #If the current node is not the goal then we expand the node that we took out from fringe
        if node.get_state() != destination:

            for child_node in node_list:
                if child_node.get_state() not in visited_nodes:
